...
def calculate_volume_score(volumes: List[float]) -> float:
    ...
    if 0.8 <= ratio < 1.0:
        ...
        return 0.4 + (ratio - 1.0) * 1.2
    elif 1.5 < ratio <= 2.0:
        return 1.0 - (ratio - 1.5) * 0.8
    elif ratio > 2.0:
        return max(-0.5, 0.6 - (ratio - 2.0) * 0.5)
    else:
        return max(-1.0, (ratio - 0.8) * 2)
...
def calculate_composite_score(close_prices: List[float], volumes: List[float]) -> Dict:
    ...
    # 加权：波动率50% + 均值回归35% + 成交量15%
    # 基于IC分析，波动率因子最有效，给予更高权重
    composite = (
...
class FinalBacktest:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.params = {
            'top_n': 8,               # 减少选股数量，集中持仓
            'rebalance_interval': 20, # 调仓频率
            'stop_loss': 0.03,        # 3%止损
            'take_profit': 0.15,      # 15%止盈
            'min_score': 0.25,        # 选股门槛
            'position_size': 0.10,    # 单股仓位10%
            'max_positions': 8,       # 最大持仓
    ...
    def _simulate_trading(self, stock_data_dict: Dict,
        ...
        print(f"  单股仓位: {self.params['position_size']*100:.0f}%")
        ...
        for day in range(60, trading_days):
            ...
            if market_condition == 'bear':
                position_multiplier = 0.7  # 熊市降低仓位
...
def main():
    """主函数"""
    print("="*60)
    print("A股多因子选股策略 - 最终优化版")
    print("="*60)
    print("\n优化内容:")
    print("1. 提高波动率因子权重至50%（基于IC分析）")
    print("2. 减少选股数量至8只，集中持仓")
    print("3. 增加动态仓位管理（根据市场环境调整）")
    print("4. 优化因子评分函数的非线性映射")
    ...
    if result:
        ...
        result_file = os.path.join(result_dir, f"final_strategy_{timestamp}.json")
